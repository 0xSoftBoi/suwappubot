...
## 5. Bridge Transaction Durability (crash-mid-bridge recovery)

- **Design intent is correct**: `bot/models/bridge.py:14-17` — "The row is created at BUILD time, before anything is signed... a transaction that exists on chain with no row is invisible forever, whereas a row whose transaction was never signed is harmless and expires on its own." Confirmed in practice: `api/webapp.py:3806` creates the `BridgeTransfer` row (state defaults to `pending_broadcast`, `bot/models/bridge.py:63`) before `api/webapp.py:3861` `/bridge/record` transitions it to `source_pending` once the client reports a broadcast tx hash.
- **CCTP relayer bookkeeping is separately durable and resumable**: `bot/models/cctp.py:70-77` (`CctpGenericDeposit.status`) explicitly documents crash recovery: `pending_broadcast` rows are written by `record_burn` **before** `send_raw_transaction` (per the comment referencing `swap_engine._execute_cctp_swap`), then reconciled against the source-chain receipt on next relayer pass and "promoted to burned or dropped" — i.e. a crash between DB-write and broadcast self-heals by checking the chain, not by trusting the DB row alone. `cctp_generic_relayer.py:337` (`_reconcile_pending_broadcasts`) is the resumption sweep. Claim/lease columns (`claimed_at`/`claimed_by`, `cctp.py:92-93`) prevent two relayer replicas from double-broadcasting the same `receiveMessage` after a crash+restart — good multi-replica safety.
- **Gap found**: `bridge_transfers` (the user-facing table, `bot/models/bridge.py`) has **no relayer/sweeper service found** in `bot/services/` (grep for `bridge_transfers`/`BridgeTransfer` across the repo returns only `database/db.py`, `api/webapp.py`, and the model file itself — zero hits in `bot/services/*.py`). The model's own docstring (`bot/models/bridge.py:8-12`) says this row's state is *derived* from the underlying provider's own bookkeeping table (e.g. CCTP relayer rows) "rather than duplicated," which is sound *if* something actually re-derives and writes it back. Based on the grep, that re-derivation appears to happen only synchronously, on-demand, when the user hits `GET /bridge/transfers/{id}` (`api/webapp.py:3900-3946`, `_bridge_transfer_response` derives `state` at read time) — there is no background sweep that proactively advances/finalizes `bridge_transfers` rows for non-CCTP providers, or that would surface a stuck transfer to an operator without a user pulling the API.
- **Risk**: MEDIUM. Funds are not directly at risk for the CCTP path (self-healing relayer with its own durable state machine, confirmed above). But for the generic `bridge_transfers` table across "any provider" (per the model docstring, this isn't CCTP-only), if the state is only ever recomputed on user read, a transfer that stalls while the user isn't looking (app closed, notification not delivered) has no operator-side alert and no automatic recovery attempt — it just sits until the user reopens the app. Given `Index("ix_bridge_transfers_state_updated", "state", "updated_at")` (`bot/models/bridge.py:78`) already exists specifically to support a "sweep for rows still in flight" (per the model's own comment, `bridge.py:75-76`), the sweep the index was built for does not appear to exist yet.
- **Fix**: Delegate to `db-migrate`/`bot-dev`: add a periodic background sweeper (same shape as `fee_sweeper`/`cctp_generic_relayer`) that queries `bridge_transfers WHERE state NOT IN ('complete','failed') AND updated_at < now() - interval` and either re-derives state from the provider or pages an operator after N hours stalled. This closes the one durability gap in an otherwise well-designed recovery architecture.

---

*Audit scope note*: static read-only review of `bot/models/`, `api-ts/src/db/schema/`, `database/db.py`, and targeted logging greps. Did not run the app, did not query a live database to confirm actual column types/constraints in production (e.g., whether `recovery_email` truly lacks a unique index in prod Postgres vs just in the migration source) — recommend `db-migrate` confirm live schema state before applying fixes. Did not audit `api-ts` logging exhaustively (spot-checked only).
