...
| 2 | WebGL figures are hidden below 1024px by CSS (no blank canvases; the slot collapses). Lower priority than first thought | site.css:1304 | Vercel Ship: SVG placeholder, canvas swapped in after compile (03 §1) | Static/CSS fallback per GL component; lazy-init after first paint; gate on pointer and width |
...
| 7 | No logo wall or quantified proof (the local "Quote unavailable" ticket is only the missing demo key; production quotes live) | render of 2026-09-02; 00 §6 | Notion percentile stat; Cognition enterprise wall; Suno press wall | Monochrome strip of chains, venues, integrations we route through; one quantified case study; a single percentile or volume stat under the headline |
