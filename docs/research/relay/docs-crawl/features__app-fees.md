...
Source: https://docs.relay.link/features/app-fees
...
If you include the appFees parameter on an unsupported transaction type (such as sends, transfers, or gasless execution without a prior quote), the API will not return an error — but no fees will be collected. Always verify fee collection is working using the monitoring endpoints described below.
​
Supported Transaction Types
...
The easiest way to claim your App Balance is to use the Relay App. The Relay App Balance page also supports withdrawing via a multisig. Here’s how you can withdraw via the API:
​
Get App Balance
...
To learn more about this api, see the Get App Fee Balance API reference.
​
How to withdraw your off-chain balance?
...
Once done, you can check your balance again to verify the withdrawal. To learn more about this api, see the Claim App Fee Balance API reference.
​
Withdrawing via a smart contract
