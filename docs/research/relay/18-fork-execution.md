...
Fix (`swap_engine.py`, `_execute_relay_swap`): a Relay quote is an order. Its deposit calldata carries the order id and the solver is bound to the quoted `minimumAmount` (it refunds rather than under-fills). So while the approved quote is fresh (within 90 s), the executor signs exactly the steps the user saw, as Relay's own SDK does. Re-quoting with the strict guard remains the fallback for an expired quote or one that lost its steps in transit (agent/webapp rehydration). Every step-validation check still runs on the approved steps. Two tests cover the new branch; the fork run then passed on the first try.

A follow-up money-path review of that change found a real hole it opened: on the agent API path the quote's `raw_quote` is caller-supplied, so a forged step list with a fresh timestamp could have been signed. Closed by signing the step list with an HMAC (keyed with the server encryption key) at quote time and executing directly only when the signature verifies; unsigned or mismatched steps fall back to the re-quote branch. The same review drove: tz-normalising rehydrated timestamps (they were naive and crashed the comparison), persisting the Relay `request_id` before the deposit is broadcast, a 12-hour wall-clock ceiling so an unfilled intent cannot sit in CONFIRMING forever, persisting refund reasons, and never letting an origin hash stand in for the destination fill hash.
