...
- Status: `GET /intents/status/v3?requestId=0x…` → `{status: waiting|depositing|pending|submitted|delayed|success|failure|refund, txHashes?, inTxHashes?, failReason?, refundFailReason?}` (treat anything not success/failure/refund as pending).
