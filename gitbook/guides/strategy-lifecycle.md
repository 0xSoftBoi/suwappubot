...
For time-based recurring purchases specifically, the [Recurring DCA guide](dca-automation.md) adds schedule-slot identity, daylight-saving deduplication, missed-slot policy, and the rule that an unresolved prior economic action blocks the next installment.
