...
This guide targets **CrewAI 1.15.x** and Python `>=3.10,<3.14`, and mirrors the maintained Suwappu CrewAI product's `2.1.x` operating contract. CrewAI's current custom-tool API supports native async functions, so you do not need to wrap the async Suwappu Python SDK in thread pools.
...
## Put every API call behind an operational boundary

The maintained product does not let individual tools invent their own timeout, retry, or logging behavior. It wraps every Suwappu SDK operation in one host-owned runtime boundary:

- a **25-second operation deadline** by default, configurable with `SUWAPPU_OPERATION_TIMEOUT_SECONDS` from `0.01` through `30` seconds;
- typed transport and protocol failures so callers do not parse exception strings;
- response validation for critical identifiers before recording success; and
- one metadata-only event with operation, outcome, duration, and optional HTTP status.

That event stream deliberately excludes prompts, tool arguments, credentials, wallet addresses, quote/swap IDs, bodies, and exception messages. Add tenant/run correlation in your own logging context instead of turning customer identifiers into metric labels.

Do not put a generic retry decorator around financial tools. A bounded retry can be reasonable for a stale-tolerant read. A quote should normally be refreshed. A managed submission is different: once submission begins, some failures make the economic outcome **unknown**, so reconciliation has to precede any retry.
...
Once managed submission begins, a timeout, network failure, HTTP 408, 5xx, or malformed successful response does not prove that nothing happened. The maintained host represents these cases with a dedicated outcome-unknown error carrying the original approval/intent context, rather than flattening them into an ordinary tool failure.
...
## Operate each customer as an isolated tenant

A production SaaS should resolve its Suwappu credential, wallet policy, approval policy, concurrency limit, spend budget, and model-token budget from authenticated tenant context. Never ask a model to choose which customer's credential to use.

The maintained Crew starts with CrewAI `cache=False`, `memory=False`, and `share_crew=False`. Financial observations age quickly, and agent memory is not an acceptable accidental cross-tenant data store. If you enable CrewAI memory, caching, or sharing, perform a new isolation/retention review first.

For long-running products, CrewAI Flows can persist workflow state and provide human-feedback checkpoints. Keep the durable **economic intent** in your own database anyway. At minimum persist tenant, exact quote, approval actor/time, durable idempotency key, simulation reference, attempt timestamps, returned swap ID when known, reconciliation state, and terminal result.

Reserve separate capacity for reconciliation. A traffic spike that fills your normal analysis queue must not prevent the service from discovering whether an already-submitted trade moved money.

## Treat model tracing as sensitive customer data

CrewAI tracing can be useful for diagnosing agent decisions, task timelines, tool use, and model calls, but those records can contain customer strategy or wallet context. Before turning tracing on, define redaction, access control, retention, tenant isolation, deletion/export behavior, and which tool arguments are permitted to leave your service.

Keep API keys and approval credentials out of prompts and trace attributes. The narrow Suwappu operation events described above are designed for service telemetry; they are not a substitute for deliberately governed model traces.

## Know the current ChromaDB dependency exception

As of this guide's CrewAI 1.15.x baseline, CrewAI installs ChromaDB 1.1.x. GitHub's reviewed [CVE-2026-45829 advisory](https://github.com/advisories/GHSA-f4j7-r4q5-qw2c) covers a critical pre-authentication code-injection issue in the Chroma HTTP server and currently lists no patched version.

The maintained Suwappu CrewAI product does **not** start or expose a Chroma server and disables Crew memory, so its dependency gate carries one narrowly documented audit exception for that unreachable server path. That exception is invalid if your deployment enables Crew memory, launches/exposes Chroma's server or API, or otherwise makes the affected server code reachable. Re-evaluate on every CrewAI/Chroma update and remove the exception as soon as a supported fixed dependency path exists.
...
- Put Suwappu calls behind a finite host operation deadline and metadata-only telemetry.
...
- Treat timeout/network/408/5xx/malformed-success submissions as outcome-unknown and reconcile before retrying.
- Keep Crew cache/memory/sharing off unless tenant isolation and data retention have been deliberately reviewed.
- Rate-limit and budget per tenant; reserve reconciliation capacity.
- Govern model traces as sensitive customer data.
- Gate releases on lint/format, behavior tests, dependency audit, package build, and clean-install smoke tests.
- Regression-test prompts/tool calls for unauthorized execution, stale quotes, ambiguous wallets, malformed responses, and reconciliation failures before live-money promotion.
