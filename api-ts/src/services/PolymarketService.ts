...
/**
 * CLOB L2 auth signature.
 *
 * Must match Polymarket's reference implementation EXACTLY or every
 * authenticated request 401s. Ground truth is py-clob-client
 * `signing/hmac.py::build_hmac_signature`:
 *
 *     base64_secret = base64.urlsafe_b64decode(secret)
 *     h = hmac.new(base64_secret, timestamp + method + requestPath + body, sha256)
 *     return base64.urlsafe_b64encode(h.digest())
 *
 * i.e. the secret is base64URL-**decoded to raw bytes** before keying, and the
 * digest is base64URL-encoded. This previously keyed the HMAC with the raw
 * secret STRING and emitted standard base64 — both wrong, so no authenticated
 * CLOB call from api-ts could ever have been accepted.
 *
 * The SDK's `.replace("'", '"')` is a Python-only artifact (Python's `str(dict)`
 * emits single quotes); `JSON.stringify` already emits double quotes, so the
 * body string is passed through unchanged.
 *
 * Exported so tests can pin it against vectors generated by the Python SDK.
 */
export function buildClobHmacSignature(
	...
	const hmac = crypto.createHmac('sha256', Buffer.from(secret, 'base64url'))
	hmac.update(message)
	// Python's base64.urlsafe_b64encode KEEPS '=' padding; Node's 'base64url'
	// encoding STRIPS it. The server compares the string, so a missing trailing
	// '=' is enough to fail auth — translate the alphabet by hand and keep the
	// padding rather than using digest('base64url').
...
export function buildClobOrderBody(signed: SignedClobOrder, owner: string): {
	...
	// The POSTed body must serialize the SAME struct that was EIP-712 signed,
	// field-for-field, or the CLOB recovers a different hash and rejects the order.
	//
	// This previously sent the v1 shape — taker/expiration/nonce/feeRateBps, which
	// CLOB V2 DROPPED — and omitted v2's timestamp/metadata/builder, which ARE in
	// the signed struct. So the body could never match the signature.
	// v2 schema: ORDER_TYPEHASH 0xbb86318a… (Polymarket/ctf-exchange-v2
	// src/exchange/libraries/Structs.sol); see lib/polymarket-eip712.ts.
