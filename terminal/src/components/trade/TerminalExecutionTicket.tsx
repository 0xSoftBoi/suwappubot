...
import { TerminalButton, TerminalSelectPill, TerminalTokenPill } from '../foundation/TerminalControls'
  ...
  return (
    <button
      ...
      className={joinClasses(
        'terminal-theme-control flex-1 px-4 py-2 text-sm font-semibold transition-colors',
...
function ExecutionModeTabs({
  ...
  const options: Array<{ id: TerminalExecutionMode; label: string; meta: string }> = [
    ...
    <div className="terminal-theme-inset inline-flex flex-wrap gap-1 p-1">
      {options.map((option) => (
        <button
          ...
          className={joinClasses(
            'px-3 py-2 text-left transition-colors [border-radius:var(--terminal-radius-card)]',
            mode === option.id
        ...
        >
          ...
          <div className="terminal-theme-caption text-[10px] uppercase opacity-70">{option.meta}</div>
        </button>
      ))}
    </div>
  )
...
function AssetLeg({
  ...
  return (
    <div className="terminal-theme-card p-4">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0 flex-1">
          <div className="terminal-theme-caption text-[10px] uppercase text-terminal-text-muted">{label}</div>
          <input
            value={amount}
            onChange={(event) => onAmountChange?.(event.target.value)}
            readOnly={readOnly}
            className="mt-2 w-full bg-transparent font-mono text-[28px] leading-none text-terminal-text outline-none placeholder:text-terminal-text-muted"
            placeholder="0.0"
          />
          <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-terminal-text-secondary">
            <span>
              Balance {token.balance ? `${token.balance} ${token.symbol}` : '--'}
            </span>
        ...
        <div className="grid justify-items-end gap-2">
          <TerminalTokenPill symbol={token.symbol} label={token.name} />
    ...
    return (
      <TerminalInset className="grid gap-3">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <div className="terminal-theme-control px-3 py-3">
            <div className="terminal-theme-caption text-[10px] uppercase text-terminal-text-muted">Limit price</div>
            <input
              value={limitPrice}
              onChange={(event) => onLimitPriceChange(event.target.value)}
              className="mt-2 w-full bg-transparent font-mono text-lg text-terminal-text outline-none"
              placeholder="0.00"
            />
          </div>
          <div className="grid gap-2">
            <div className="terminal-theme-caption text-[10px] uppercase text-terminal-text-muted">Expiry</div>
  ...
  if (mode === 'dca') {
    return (
      <TerminalInset className="grid gap-3">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <div className="terminal-theme-control px-3 py-3">
            <div className="terminal-theme-caption text-[10px] uppercase text-terminal-text-muted">Total budget</div>
            <input
              value={totalBudget}
          ...
          <div className="terminal-theme-control px-3 py-3">
            <div className="terminal-theme-caption text-[10px] uppercase text-terminal-text-muted">Orders</div>
            <input
              value={orderCount}
        ...
        <TerminalKeyValueRow
          ...
          detail="Projected notional for each scheduled execution."
        />
      </TerminalInset>
    )
  }
    ...
    <TerminalInset className="grid gap-3">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button
          ...
          className="terminal-theme-caption text-[10px] uppercase text-terminal-text-secondary transition-colors hover:text-terminal-text"
        >
      ...
      {showTpSl ? (
        <div className="grid gap-3 md:grid-cols-2">
          <div className="terminal-theme-control px-3 py-3">
            <div className="terminal-theme-caption text-[10px] uppercase text-terminal-text-muted">Take profit</div>
            <input
              value={tpPrice}
          ...
          <div className="terminal-theme-control px-3 py-3">
            <div className="terminal-theme-caption text-[10px] uppercase text-terminal-text-muted">Stop loss</div>
            <input
              value={slPrice}
...
function RouteSummary({ quote, mode }: { quote?: SwapQuote; mode: TerminalExecutionMode }) {
    ...
    <TerminalInset className="grid gap-3">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-2">
          <TerminalStatusPill tone={tone}>{routeLabel(quote)}</TerminalStatusPill>
          {quote ? <TerminalChainBadge chain={quote.fromToken.chain} /> : null}
          {quote && quote.fromToken.chain !== quote.toToken.chain ? <TerminalChainBadge chain={quote.toToken.chain} /> : null}
        </div>
        <div className="terminal-theme-caption text-[10px] uppercase text-terminal-text-muted">
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <TerminalMetricCard
          label={mode === 'swap' ? 'Expected out' : mode === 'limit' ? 'Target out' : 'Projected out'}
        ...
        <TerminalMetricCard
          ...
          detail={quote ? formatDuration(quote.estimatedDuration) : 'Route timing'}
        />
      </div>
      ...
      <div className="grid gap-2">
        <TerminalKeyValueRow
          ...
          detail="Execution ratio from the current route."
        />
        <TerminalKeyValueRow
          label="Min received"
          value={quote ? `${quote.minReceived} ${quote.toToken.symbol}` : '--'}
          detail="Protected output after slippage."
        />
        <TerminalKeyValueRow
          label="Expires"
          value={quote ? new Date(quote.expiresAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '--'}
          detail="Route validity window."
        />
      </div>
    </TerminalInset>
  )
...
function primaryLabel({
  ...
  if (isBusy) return mode === 'swap' ? 'Staging route...' : mode === 'limit' ? 'Placing order...' : 'Scheduling...'
  if (mode === 'swap') return `${side === 'buy' ? 'Buy' : 'Sell'} ${token.symbol}`
    ...
    <div className="grid gap-4">
      <div className="grid gap-3 xl:grid-cols-[0.72fr_1.28fr]">
        <div className="flex gap-2">
          <ExecutionToggle active={buyMode} onClick={() => onSideChange('buy')} tone="bull">
            Buy
          </ExecutionToggle>
          <ExecutionToggle active={!buyMode} onClick={() => onSideChange('sell')} tone="bear">
            Sell
          </ExecutionToggle>
        </div>
      ...
      <div className="grid gap-3">
        <AssetLeg
          ...
          detail={mode === 'dca' ? 'Total schedule budget' : 'Operator entry size'}
        />

        <div className="flex justify-center">
          <button
            onClick={onFlip}
            className="terminal-theme-control inline-flex h-10 w-10 items-center justify-center text-terminal-text-secondary hover:text-terminal-text"
        ...
        <AssetLeg
          label={mode === 'swap' ? 'To' : mode === 'limit' ? 'Target asset' : 'Accumulating'}
          token={toToken}
          amount={inputValue(resultAmount)}
          usdValue={mode === 'dca' ? safeNumber(totalBudget) ?? undefined : quote?.toAmountUsd}
          readOnly
          detail={resultDetail}
        />
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-[1fr_auto] md:items-center">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <TerminalStatusPill tone="neutral">{slippage}% slippage</TerminalStatusPill>
          {mode === 'swap' && showTpSl ? <TerminalStatusPill tone="warm">TP / SL armed</TerminalStatusPill> : null}
        </div>
