      ...
      <table className="w-full text-xs">
        <thead>
          <tr className="text-terminal-text-muted border-b border-terminal-border">
            ...
            <th className="text-right py-1.5 px-2 font-medium">1h Change</th>
        ...
        <tbody>
          {pools.length === 0 && (
            <tr>
              <td colSpan={9} className="text-center text-terminal-text-muted text-sm py-8">
                No trending pools found
              </td>
            </tr>
          )}
          {pools.map((pool, idx) => {
            const changeH1 = formatChange(pool.priceChangeH1)
