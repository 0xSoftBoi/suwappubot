...
function DetailRow({ label, value, danger }: { label: string; value: string; danger?: boolean }) {
  return (
    <div className="flex items-center justify-between gap-3">
      ...
      <span className={`text-[10px] font-mono ${danger ? 'text-red-400 font-semibold' : 'text-terminal-text'}`}>
        {value}
      </span>
    </div>
  )
...
export function SecurityBadge({ security, loading, compact }: SecurityBadgeProps) {
    ...
    <div className="relative inline-block" ref={tooltipRef}>
      <button
        ...
        className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-[10px] border transition-all cursor-pointer ${config.borderClass} ${config.bgClass} ${config.textClass} hover:brightness-125 ${
          isDanger ? 'animate-pulse-slow shadow-[0_0_6px_rgba(239,68,68,0.3)]' : ''
        }`}
      >
        {/* Trust score number */}
        <span className={`inline-flex items-center justify-center w-4 h-4 rounded-full text-[9px] font-bold ${config.scoreBg}`}>
          {score}
        </span>
        {!compact && (
          <>
        ...
        {/* Expand indicator */}
        <svg className={`w-2.5 h-2.5 transition-transform ${expanded ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 10 10" stroke="currentColor" strokeWidth="2">
          <path d="M2 4l3 3 3-3" />
        </svg>
      </button>
      ...
      {/* Expanded tooltip */}
      {expanded && (
        <div className={`absolute z-50 top-full mt-1 right-0 min-w-[200px] rounded-lg border shadow-xl ${
          isDanger
          ...
          <div className={`flex items-center justify-between px-3 py-2 border-b ${
            ...
            <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-medium ${config.borderClass} ${config.bgClass} ${config.textClass}`}>
              <span className={`w-1.5 h-1.5 rounded-full ${config.dotClass}`} />
              {config.label}
            </span>
          </div>
          ...
          {isDanger && (
            <div className="mx-3 mb-2 px-2 py-1.5 rounded bg-red-500/15 border border-red-500/30 text-[10px] text-red-400 font-medium">
              Warning: This token has multiple risk flags. Trade with extreme caution.
            </div>
          )}
        </div>
      )}
