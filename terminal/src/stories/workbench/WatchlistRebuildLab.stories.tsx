...
function WatchlistLab({ state }: { state: LabState }) {
  ...
  const [selectedAddress, setSelectedAddress] = useState(
    seedRows[0].token.address,
  );
  const [rows, setRows] = useState<WatchlistRow[]>(
    state === "empty" ? [] : seedRows,
  );
  ...
  const activeCount = rows.filter(
    (row) => (row.priceData.change24h ?? 0) >= 0,
  ).length;
      ...
      <div className="mx-auto grid max-w-7xl gap-2.5">
        <TerminalPanel elevated>
          <TerminalPanelHeader
            ...
            description="Compact market list rebuilt without live storage or hooks."
            meta={
              <TerminalMetricCard
                label="Rows"
          ...
          <div className="grid gap-2.5 xl:grid-cols-[1.32fr_0.68fr] xl:items-start">
            <TerminalInset className="p-[var(--terminal-space-card)]">
              <div className="flex flex-col gap-2 border-b border-terminal-border pb-2 md:flex-row md:items-center md:justify-between">
                <TerminalTextField
                  value={query}
            ...
            <TerminalInset className="p-[var(--terminal-space-card)]">
              <div className="text-[9px] uppercase tracking-[0.22em] text-terminal-text-muted">
                Inspector
              </div>
              <div className="mt-2 grid gap-2">
                <TerminalMetricCard
                  ...
                  value={
                    filteredRows.find(
                      (row) => row.token.address === selectedAddress,
                    )?.token.symbol ?? "--"
                  }
                  tone="warm"
                />
                <TerminalKeyValueRow
                  ...
                  detail="Search, rows, and empty state should stay separate."
                />
                <TerminalKeyValueRow
                  label="Reusable"
                  value="search, tabs, rows"
                  detail="Portable into discovery rails and market lists."
                />
                <TerminalKeyValueRow
                  label="Next"
                  value="detail drawer"
                  detail="Build the selected-token side panel next."
                />
              </div>
            </TerminalInset>
          </div>
