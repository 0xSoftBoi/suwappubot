...
from bot.models.custodial import (
    HotWallet, GasSponsorshipConfig, UserGasUsage,
    CustodialTransaction, TransactionType, TransactionStatus
)
...
class PaymasterService:
    ...
    def get_sponsorship_config(self, chain: str) -> Optional[GasSponsorshipConfig]:
        ...
        with get_session() as session:
            return session.query(GasSponsorshipConfig).filter(
                GasSponsorshipConfig.chain == chain
            ).first()
    ...
    def set_sponsorship_config(
        ...
        with get_session() as session:
            config = session.query(GasSponsorshipConfig).filter(
                GasSponsorshipConfig.chain == chain
            ).first()
        ...
        with get_session() as session:
            return session.query(GasSponsorshipConfig).filter(
                GasSponsorshipConfig.id == config_id
            ).first()
    ...
    def get_user_daily_gas_usage(self, user_id: int, chain: str) -> float:
        ...
        with get_session() as session:
            usage = session.query(UserGasUsage).filter(
    ...
    def can_sponsor_gas(
        ...
        if estimated_gas_usd > config.max_gas_per_tx_usd:
            return False, f"Gas cost ${estimated_gas_usd:.2f} exceeds max ${config.max_gas_per_tx_usd:.2f} per transaction"
    ...
    async def sponsor_transaction(
        ...
        """
        # Get gas estimate
        gas_limit, gas_cost_native, gas_cost_usd = await self.estimate_gas_cost(
            chain_name, tx_data
        )
        ...
        try:
            ...
            return True, f"Gas sponsored: {float(amount_to_send):.6f} {get_chain_by_name(chain_name).native_token}", tx_hash
