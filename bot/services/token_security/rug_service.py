...
class RugService:
                ...
                async with websockets.connect(self._ws_url) as ws:
                    ...
                    while self._running:
                        ...
                        if "params" in data:
                            ...
                            if any("withdraw" in log.lower() or "removeliquidity" in log.lower() for log in logs):
    ...
    async def _get_users_holding_token(self, token_mint: str) -> List[tuple]:
        ...
        with get_session() as session:
            ...
            results = session.query(User.id, Wallet.id).join(Wallet).join(SwapTransaction, SwapTransaction.user_id == User.id).filter(
                User.panic_sell_enabled == True,
                SwapTransaction.to_token == token_mint, # Assuming token_mint is symbol or address
