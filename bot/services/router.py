...
class SmartRouter:
    ...
    async def get_all_routes(
        ...
        if (
            is_same_chain
            and from_chain.lower() == "tempo"
            and self.tempo_dex.is_supported_pair(from_token, to_token)
        ):
            tasks.append(self._get_tempo_dex_route(from_token, to_token, from_amount, from_address))
        ...
        if (
            is_usdc
            and not is_same_chain
            and self.cctp.is_supported_route(from_chain, to_chain, "USDC")
        ):
            tasks.append(self._get_cctp_route(from_chain, to_chain, from_amount))
        ...
        if is_solana_route and not is_same_chain:
            if self.wormhole.is_supported_route(from_chain, to_chain, from_token):
                tasks.append(
                    self._get_wormhole_route(from_chain, to_chain, from_token, from_amount)
                )

        # ============================================================
        # PRIORITY 6.5: New bridge providers (NEAR Intents, Allbridge,
        # Symbiosis, Arbitrum native) - non-EVM reach + canonical/solver
        # routes not covered above.
        # ============================================================
        if not is_same_chain:
            tasks.append(
                self._get_bridge_routes(
                    from_chain,
                    to_chain,
                    from_token,
                    from_amount,
        ...
        if is_solana_source and is_solana_dest:
            tasks.append(
                self._get_jupiter_route(from_token, to_token, from_amount, from_address, slippage)
            )
    ...
    async def _get_bridge_routes(
        ...
        """Get the best route from the new bridge provider registry
        (NEAR Intents, Allbridge, Symbiosis, Arbitrum native).

        These providers race each other internally via get_bridge_quotes();
        only the single best (highest to_amount) quote is surfaced here as
        one RouteOption, matching the one-task-per-RouteOption pattern used
        by the other _get_*_route helpers.
        """
        try:
            # `slippage` here is a percent (router.py convention, e.g. 0.5 =
            # 0.5%); the bridge provider ABC takes basis points.
            ...
            amount_int = int(quote.to_amount)
            decimals = get_token_decimals(quote.to_token, to_chain) or 18
            to_amount_human = amount_int / (10**decimals)

            prices = await price_service.get_prices([quote.to_token])
            to_price = prices.get(quote.to_token.upper(), 0)
            output_usd = to_amount_human * to_price

            from_decimals = get_token_decimals(token, from_chain) or 18
            ...
            return RouteOption(
                ...
                provider_display=quote.provider.replace("_", " ").title(),
                from_chain=from_chain,
                from_token=token,
                from_amount=from_amount,
                from_amount_human=from_amount_human,
                ...
                raw_quote={
                    ...
                    # Nested, never spread: raw provider JSON must not be able
                    # to override our derived fields above (e.g. a remote
                    # payload containing its own "transaction_request" or
                    # "deposit_address" key could otherwise silently redirect
                    # funds).
                    "raw": quote.raw_response,
                },
            )
        except Exception as e:
            logger.debug(f"Bridge registry route error: {e}")
            return None
    ...
    def format_routes_comparison(self, routes: List[RouteOption]) -> str:
        ...
        for i, route in enumerate(routes, 1):
            ...
            time_str = (
                f"{route.execution_time_seconds // 60}m"
                if route.execution_time_seconds >= 60
                else f"{route.execution_time_seconds}s"
            )
