...
class AcrossAPI:
    ...
    async def get_quote(
        ...
        if not self.is_supported_route(from_chain, to_chain, token):
            raise AcrossError(f"Route not supported: {token} from {from_chain} to {to_chain}")
        ...
        async with session.get(f"{self.api_url}/suggested-fees", params=params) as response:
    ...
    async def get_limits(
        ...
        async with session.get(f"{self.api_url}/limits", params=params) as response:
            if response.status != 200:
                error_text = await response.text()
    ...
    async def get_hypercore_usdc_deposit(
        ...
        approval_txns = [
            self._normalize_tx(t, origin_chain_id) for t in (data.get("approvalTxns") or []) if t
        ]
    ...
    def build_deposit_calldata(
        ...
        spoke_pool = Web3().eth.contract(
            address=Web3.to_checksum_address(quote.spoke_pool), abi=DEPOSIT_V3_ABI
        )
        ...
        exclusive_relayer = quote.raw_quote.get(
        ...
        is_eth = (
            quote.from_token.upper() in ["ETH", "WETH"]
