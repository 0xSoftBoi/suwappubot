...
class OrdersFlow(BaseWhatsAppFlow):
    ...
    async def _step_main_menu(self, user_id: str, user_db_id: int, text: str, state: ConversationState) -> FlowResponse:
        ...
        if text == "order_create":
            await self._update(user_id, "lo_pair")
            # Common pairs
            pairs = [
                ("ETH/USDC", "eth_usdc"), ("BTC/USDC", "btc_usdc"),
                ("SOL/USDC", "sol_usdc"), ("ARB/USDC", "arb_usdc"),
                ("LINK/USDC", "link_usdc"), ("UNI/USDC", "uni_usdc"),
                ("OP/USDC", "op_usdc"), ("AAVE/USDC", "aave_usdc"),
            ]
            rows = [{"id": f"pair_{pid}", "title": name} for name, pid in pairs]
            return FlowResponse(
                text="Select the trading pair:",
                header="📋 New Limit Order",
    ...
    async def _step_lo_pair(self, user_id: str, user_db_id: int, text: str, state: ConversationState) -> FlowResponse:
    ...
    async def _step_lo_trigger(self, user_id: str, user_db_id: int, text: str, state: ConversationState) -> FlowResponse:
        try:
        ...
        return FlowResponse(text=f"Trigger price: *${price}*\n\nEnter the amount of {state.data.get('from_token', 'tokens')} to swap:")

    async def _step_lo_amount(self, user_id: str, user_db_id: int, text: str, state: ConversationState) -> FlowResponse:
        try:
        ...
        return FlowResponse(
            text=(
                ...
                f"Sell {amount} {from_token} → {to_token}\n"
    ...
    async def _step_lo_confirm(self, user_id: str, user_db_id: int, text: str, state: ConversationState) -> FlowResponse:
    ...
    async def _step_cancel_select(self, user_id: str, user_db_id: int, text: str, state: ConversationState) -> FlowResponse:
        await self._clear(user_id)
        db_uid = state.data.get("user_db_id") or user_db_id
        try:
...
class DCAFlow(BaseWhatsAppFlow):
    ...
    async def _step_main_menu(self, user_id: str, user_db_id: int, text: str, state: ConversationState) -> FlowResponse:
        ...
        if text == "dca_create":
            ...
            pairs = [
                ("USDC → ETH", "usdc_eth"), ("USDC → BTC", "usdc_btc"),
                ("USDC → SOL", "usdc_sol"), ("USDC → LINK", "usdc_link"),
                ("USDC → ARB", "usdc_arb"), ("USDC → OP", "usdc_op"),
            ]
    ...
    async def _step_dca_pair(self, user_id: str, user_db_id: int, text: str, state: ConversationState) -> FlowResponse:
        ...
        return FlowResponse(text=f"Pair: *{from_token} → {to_token}*\n\nEnter the amount of {from_token} per execution:")

    async def _step_dca_amount(self, user_id: str, user_db_id: int, text: str, state: ConversationState) -> FlowResponse:
        try:
            amount = float(text.replace(",", "").strip())
            if amount <= 0:
                raise ValueError
    ...
    async def _step_dca_interval(self, user_id: str, user_db_id: int, text: str, state: ConversationState) -> FlowResponse:
        interval_map = {
            "dca_daily": 24,
            "dca_weekly": 168,
            "dca_monthly": 720,
        ...
        await self._update(user_id, "dca_confirm", {"interval_hours": hours, "interval_label": label})
    ...
    async def _step_dca_confirm(self, user_id: str, user_db_id: int, text: str, state: ConversationState) -> FlowResponse:
    ...
    async def _step_dca_manage_select(self, user_id: str, user_db_id: int, text: str, state: ConversationState) -> FlowResponse:
        await self._clear(user_id)
        db_uid = state.data.get("user_db_id") or user_db_id
        try:
