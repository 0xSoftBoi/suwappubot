...
class GcpKmsClient(KmsClientBase):
    ...
    def decrypt(self, ciphertext: bytes) -> bytes:
        ...
        response = self._client.decrypt(request={"name": self._key_name, "ciphertext": ciphertext})
        return response.plaintext
...
def get_kms_client() -> KmsClientBase:
    ...
    if provider == "dev":
        ...
        if not all([settings.gcp_project_id, settings.gcp_kms_keyring, settings.kms_key_id]):
            raise ValueError("GCP KMS requires gcp_project_id, gcp_kms_keyring, and kms_key_id")
