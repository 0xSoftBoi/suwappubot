...
class MetricsService:
    ...
    def get_overview(self) -> dict:
        ...
        with get_session() as session:
            ...
            active_today = (
                session.query(User)
                .filter(User.updated_at >= datetime(today.year, today.month, today.day))
                .count()
            )

            week_ago = datetime.now(timezone.utc) - timedelta(days=7)
            active_week = session.query(User).filter(User.updated_at >= week_ago).count()

            # Transaction counts
            total_swaps = session.query(SwapTransaction).count()
            completed_swaps = (
                session.query(SwapTransaction).filter(SwapTransaction.status == "completed").count()
            )
            failed_swaps = (
                session.query(SwapTransaction).filter(SwapTransaction.status == "failed").count()
            )

            # Today's swaps
            today_swaps = (
                session.query(SwapTransaction)
                .filter(SwapTransaction.created_at >= datetime(today.year, today.month, today.day))
                .count()
            )
    ...
    def get_volume_stats(self, days: int = 30) -> dict:
        ...
        with get_session() as session:
            ...
            return {
                ...
                "by_token": {
                    k: float(v)
                    for k, v in sorted(volume_by_token.items(), key=lambda x: x[1], reverse=True)[
                        :10
                    ]
                },
    ...
    def get_top_users(self, limit: int = 10) -> List[dict]:
        ...
        with get_session() as session:
            ...
            transactions = (
                session.query(SwapTransaction).filter(SwapTransaction.status == "completed").all()
            )
            ...
            # Sort by volume
            sorted_users = sorted(users_data.values(), key=lambda x: x["volume"], reverse=True)[
                :limit
            ]
    ...
    def get_chain_health(self) -> dict:
        ...
        with get_session() as session:
            ...
            chain_stats = defaultdict(
                lambda: {"total": 0, "completed": 0, "failed": 0, "pending": 0}
            )
            ...
            for chain, stats in chain_stats.items():
                success_rate = (
                    (stats["completed"] / stats["total"] * 100) if stats["total"] > 0 else 100
                )
                result[chain] = {
                    **stats,
    ...
    def get_recent_errors(self, limit: int = 20) -> List[dict]:
        ...
        with get_session() as session:
            failed = (
                session.query(SwapTransaction)
                .filter(SwapTransaction.status == "failed")
