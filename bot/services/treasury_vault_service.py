...
class TreasuryVaultService:
    ...
    def _ensure_usdc_approval(
        ...
        if allowance < amount_wei:
            MAX_UINT256 = 2**256 - 1
            self._build_and_send(web3, wallet, usdc.functions.approve(spender_addr, MAX_UINT256), private_key)
    ...
    def deposit_to_vault(self, usdc_amount: Decimal) -> str:
        if not getattr(settings, "aave_enabled", False):
            logger.info("[mock] deposit_to_vault %.6f USDC (vault_type=%s)",
                        usdc_amount, getattr(settings, "vault_type", "aave"))
            return "0x" + "0" * 64
        try:
            if self._is_morpho():
    ...
    def _deposit_morpho(self, usdc_amount: Decimal) -> str:
        ...
        logger.info("Deposited %.6f USDC to Morpho vault %s tx=%s", usdc_amount, vault_addr, tx_hash)
        self._update_position_after_deposit(usdc_amount)
        return tx_hash
    ...
    def _harvest_aave(self) -> Decimal:
        ...
        logger.info("harvest_aave: principal=%.6f aUSDC=%.6f yield=%.6f", principal, current_balance, yield_amount)
        return yield_amount
    ...
    def _harvest_morpho(self) -> Decimal:
        ...
        with get_session() as session:
            from bot.models.token_staking import TreasuryPosition
            pos = session.query(TreasuryPosition).filter(
                TreasuryPosition.vault_name == "morpho_base_usdc"
            ).first()
            principal = Decimal(str(pos.principal_usdc)) if pos else Decimal("0")
            if pos:
                pos.current_a_token_balance = current_balance
        ...
        yield_amount = max(current_balance - principal, Decimal("0"))
        logger.info("harvest_morpho: principal=%.6f current=%.6f yield=%.6f",
                    principal, current_balance, yield_amount)
        return yield_amount
            ...
            if getattr(settings, "aave_enabled", False):
                try:
                    ...
                    if self._is_morpho():
                        ...
                        assets_wei = vault.functions.convertToAssets(shares).call() if shares > 0 else 0
                        ...
                        with get_session() as session:
                            from bot.models.token_staking import TreasuryPosition
                            _pos = session.query(TreasuryPosition).filter(
                                TreasuryPosition.vault_name == vault_name
                            ).first()
        ...
        with get_session() as session:
            ...
            if pos:
                ...
                session.add(TreasuryPosition(
                    vault_name=vault_name, chain="base",
