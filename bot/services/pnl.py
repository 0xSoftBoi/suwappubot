...
class PnLService:
    ...
    async def get_portfolio_value(self, user_id: int) -> Dict:
        ...
        # First pass: fetch balances for every wallet and collect every held
        # token symbol so prices can be fetched in ONE batched call instead of
        # one CoinGecko round-trip per token (which used to serialize behind
        # the 1 req/sec coingecko rate limiter on the /start home path).
        wallet_balances = []
        all_tokens = set()
        for wallet in wallets:
            try:
                # get_all_balances takes the Wallet object (it reads
                # wallet.chain_type). Passing wallet.id raised AttributeError
                # for EVERY wallet — swallowed by the except below — so this
                # function silently returned $0 for everyone.
                balances = await self.wallet_service.get_all_balances(wallet)
            except Exception as e:
                logger.error(f"Portfolio value error for wallet {wallet.id}: {e}")
                continue

            wallet_balances.append((wallet, balances))
            for chain, tokens in balances.items():
                for token, balance in tokens.items():
                    if balance > 0:
                        all_tokens.add(token)

        # Deliberately NOT caught. Every token here priced at 0 would make a
        # funded portfolio report $0 — and get_portfolio_value feeds
        # save_portfolio_snapshot, which PERSISTS total_value_usd as that
        # day's historical snapshot. Swallowing a pricing outage would write a
        # permanent false $0 into the user's portfolio history and tell a user
        # with funds that their balance is zero. Failing loudly is correct
        # here: the caller shows an error, and no snapshot is written.
        prices = await price_service.get_prices(list(all_tokens)) if all_tokens else {}

        for wallet, balances in wallet_balances:
            try:
                ...
                for chain, tokens in balances.items():
                    for token, balance in tokens.items():
                        if balance > 0:
                            # get_prices() returns an explicit None for a token
                            # it cannot map to a CoinGecko id, so the key EXISTS
                            # with value None and .get(token, 0) does not
                            # default. Decimal(str(None)) raises InvalidOperation,
                            # which would abort this wallet mid-loop and leave a
                            # partially-summed total that then gets persisted.
                            raw_price = prices.get(token)
