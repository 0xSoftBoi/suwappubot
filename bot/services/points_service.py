...
from bot.models.points import (
    ...
    DailyQuest, UserQuest, JackpotPool,
    LEVELS, POINT_ACTIONS, DEFAULT_MILESTONES, DEFAULT_REWARDS, QUEST_POOL, CASHBACK_TIERS,
)
...
class PointsService:
    ...
    # ============== Quest & Jackpot Methods ==============

    def generate_daily_quests(self, date_str: str = None) -> list:
        ...
        with get_session() as session:
            ...
            existing = session.query(DailyQuest).filter(DailyQuest.date == date_str).count()
            if existing > 0:
                return session.query(DailyQuest).filter(DailyQuest.date == date_str).all()

            # Select 3 random quests
            selected = random.sample(QUEST_POOL, min(3, len(QUEST_POOL)))
    ...
    def get_today_quests(self, user_id: int) -> list:
        ...
        with get_session() as session:
            quests = session.query(DailyQuest).filter(DailyQuest.date == today).all()
            ...
            for quest in quests:
                ...
                if not user_quest:
                    user_quest = UserQuest(user_id=user_id, quest_id=quest.id)
    ...
    def update_quest_progress(self, user_id: int, quest_type: str, increment: int = 1) -> list:
        """Update progress on matching quests. Returns list of completed quest names."""
        today = datetime.utcnow().strftime("%Y-%m-%d")
        completed = []

        with get_session() as session:
            quests = session.query(DailyQuest).filter(
            ...
            for quest in quests:
                ...
                user_quest.progress = min(user_quest.progress + increment, quest.target_value)

                if user_quest.progress >= quest.target_value and not user_quest.is_completed:
    ...
    def claim_quest_reward(self, user_id: int, quest_id: int) -> tuple:
        """Claim reward for completed quest. Returns (success, points_awarded, message)."""
        with get_session() as session:
            user_quest = session.query(UserQuest).filter(
                UserQuest.user_id == user_id,
            ...
            if not user_quest.is_completed:
                return False, 0, "Quest not completed yet"
            if user_quest.claimed:
                return False, 0, "Already claimed"

            quest = session.query(DailyQuest).filter(DailyQuest.id == quest_id).first()
            if not quest:
                return False, 0, "Quest not found"

            user_quest.claimed = True
            quest_description = quest.description
            quest_points = quest.points_reward
            quest_xp = quest.xp_reward

        # Award points + XP
        total = quest_points + quest_xp
        self.award_points(user_id, "quest", amount=total, description=f"Quest: {quest_description}")

        return True, total, f"Claimed {total} points!"

    def contribute_to_jackpot(self, fee_usd: float) -> None:
        """Add 5% of fee to today's jackpot pool."""
        today = datetime.utcnow().strftime("%Y-%m-%d")
        contribution = fee_usd * 0.05

        with get_session() as session:
            pool = session.query(JackpotPool).filter(JackpotPool.date == today).first()
            if not pool:
                pool = JackpotPool(date=today, total_pool_usd=0.0)
                session.add(pool)
                session.flush()

            pool.total_pool_usd += contribution

    def draw_daily_jackpot(self, date_str: str = None) -> dict:
        """Draw jackpot winner. Returns winner info or None."""
        if date_str is None:
            date_str = datetime.utcnow().strftime("%Y-%m-%d")

        with get_session() as session:
            pool = session.query(JackpotPool).filter(
                JackpotPool.date == date_str,
                JackpotPool.is_drawn == False,
            ).first()

            if not pool or pool.total_pool_usd <= 0:
                return None

            # Get all users who swapped today (have swap points)
            today_txs = session.query(PointTransaction.user_id).filter(
                PointTransaction.action == "swap",
                PointTransaction.created_at >= datetime.strptime(date_str, "%Y-%m-%d"),
            ).distinct().all()

            if not today_txs:
                return None

            user_ids = [t.user_id for t in today_txs]
            winner_id = random.choice(user_ids)

            pool.winner_user_id = winner_id
            pool.winner_payout_usd = pool.total_pool_usd
            pool.is_drawn = True
            pool.drawn_at = datetime.utcnow()

            from bot.models.user import User
            winner = session.query(User).filter(User.id == winner_id).first()
            winner_name = winner.username if winner else f"User{winner_id}"

        return {
            "winner_id": winner_id,
            "winner_name": winner_name,
            "payout_usd": pool.total_pool_usd,
            "date": date_str,
        }
    ...
    def get_jackpot_info(self) -> dict:
        ...
        with get_session() as session:
            ...
            return {
                "date": today,
                "pool_usd": pool.total_pool_usd if pool else 0.0,
                "is_drawn": pool.is_drawn if pool else False,
            }

    def get_cashback_tier(self, user_id: int) -> dict:
        """Get user's cashback tier based on cumulative volume."""
        with get_session() as session:
            account = session.query(UserPoints).filter(UserPoints.user_id == user_id).first()
            volume = account.total_volume_usd if account else 0

        current_tier = CASHBACK_TIERS["bronze"]
        current_tier_name = "bronze"
        next_tier_name = "silver"

        for tier_name in ["diamond", "platinum", "gold", "silver", "bronze"]:
            tier = CASHBACK_TIERS[tier_name]
            if volume >= tier["volume"]:
                ...
                next_tier_name = tier_order[idx + 1] if idx < len(tier_order) - 1 else None
                break

        return {
            "tier": current_tier_name,
            "cashback_pct": float(current_tier["cashback"]),
            "volume": volume,
            "next_tier": next_tier_name,
            "next_volume": CASHBACK_TIERS[next_tier_name]["volume"] if next_tier_name else None,
        }

    def calculate_cashback(self, user_id: int, swap_amount_usd: float) -> float:
        """Calculate cashback amount for a swap."""
        tier_info = self.get_cashback_tier(user_id)
        cashback_pct = tier_info["cashback_pct"]
        return swap_amount_usd * cashback_pct / 100
