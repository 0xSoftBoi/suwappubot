...
class JitoAPI:
    ...
    async def send_bundle(
        ...
        if "error" in data:
            raise JitoError(
                f"Jito bundle error: {data['error'].get('message', 'Unknown')}",
                data["error"]
            )
    ...
    async def send_transaction(
        ...
        payload = {
            ...
            "params": [
                transaction,
                {"skipPreflight": skip_preflight, "encoding": "base64"}
            ]
        }
        ...
        if "error" in data:
            raise JitoError(
                f"Jito transaction error: {data['error'].get('message', 'Unknown')}",
                data["error"]
            )
    ...
    async def get_bundle_statuses(
        ...
        for i, bundle_id in enumerate(bundle_ids):
            ...
            if i < len(result) and result[i]:
                ...
                statuses.append(JitoBundleStatus(
                    bundle_id=bundle_id,
                    status=status,
