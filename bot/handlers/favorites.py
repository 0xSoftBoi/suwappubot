...
async def favorites_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    with get_session() as session:
        ...
        favorites = (
            ...
            .filter(FavoriteSwapPair.user_id == db_user.id)
    ...
    for fav in fav_data[:10]:  # Limit to 10
        ...
        keyboard.append(
            [
                InlineKeyboardButton(f"🔄 {fav['name']}", callback_data=f"fav_use_{fav['id']}"),
                InlineKeyboardButton("🗑️", callback_data=f"fav_delete_{fav['id']}"),
            ]
        )
