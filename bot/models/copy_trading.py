...
class TraderProfile(Base):
    ...
    def update_stats(self, pnl: float, volume: float, is_win: bool):
        ...
        self.avg_trade_size_usd = self.total_volume_usd / self.total_trades if self.total_trades > 0 else 0
...
class CopyFollow(Base):
    ...
    # Copy settings
    copy_mode = Column(String(20), default="notify")  # "notify", "auto"
...
class CopyTrade(Base):
    ...
    # Relationships
    original_swap_id = Column(Integer, ForeignKey("swap_transactions.id"), nullable=False, index=True)
    copy_swap_id = Column(Integer, ForeignKey("swap_transactions.id"), nullable=True)  # Null if pending/failed
    ...
    # Status
    status = Column(String(20), default="pending")  # "pending", "notified", "copied", "skipped", "failed"
