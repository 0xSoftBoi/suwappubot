...
def _ensure_schema(db_engine) -> None:
    ...
    _create_tenant_bot_tables(db_engine, inspector, is_sqlite)
...
def _create_tenant_bot_tables(db_engine, inspector, is_sqlite: bool) -> None:
    """Create the white-label tenant-bot tables idempotently.

    Mirrors api-ts's Drizzle schema (db/schema/tenantBots.ts) per ADR 0003 — the
    two stacks share one database and either may reach a fresh one first. Unlike
    the autopilot mirror, Python DOES read these: bot/services/tenant_bot_runtime
    serves every hosted bot's messages out of them, so a Python-first deploy
    without this would leave every tenant bot silent.

    Drizzle's Postgres enums are plain VARCHARs here — identical strings on the
    wire, and whichever service creates the table first wins. UUID primary keys
    are generated by api-ts (which owns writes to tenant_bots); Python only ever
    SELECTs and bumps counters, so no default is needed on the Python side.
    """
    try:
        tables = set(inspector.get_table_names())
    except Exception:
    ...
    if "tenant_bots" in tables:
        return

    ts = "DATETIME DEFAULT CURRENT_TIMESTAMP" if is_sqlite else "TIMESTAMP DEFAULT NOW()"
    json_type = "TEXT" if is_sqlite else "JSONB"
    dt = "DATETIME" if is_sqlite else "TIMESTAMP"
    uuid_pk = "TEXT PRIMARY KEY" if is_sqlite else "UUID PRIMARY KEY DEFAULT gen_random_uuid()"
    uuid_col = "TEXT" if is_sqlite else "UUID"

    with db_engine.begin() as conn:
        conn.execute(text(f"""
                CREATE TABLE IF NOT EXISTS tenant_bots (
                    id {uuid_pk},
                    organization_id {uuid_col} NOT NULL,
                    created_by INTEGER,
                    name VARCHAR(120) NOT NULL,
                    ...
                    bot_token_nonce VARCHAR(64),
                    bot_token_hash VARCHAR(64) UNIQUE,
                    webhook_secret VARCHAR(64),
                    status VARCHAR(20) NOT NULL DEFAULT 'draft',
                    last_error TEXT,
                    branding {json_type} NOT NULL DEFAULT '{{}}',
                    skills {json_type} NOT NULL DEFAULT '[]',
                    brief TEXT,
                    blueprint {json_type},
                    token_chain VARCHAR(32),
                    token_address VARCHAR(100),
                    token_symbol VARCHAR(32),
                    treasury_address VARCHAR(100),
                    funding_source VARCHAR(20) NOT NULL DEFAULT 'undisclosed',
                    funding_note TEXT,
                    proof_public BOOLEAN NOT NULL DEFAULT FALSE,
                    treasury_internal_user_id INTEGER,
                    treasury_internal_wallet_id INTEGER,
                ...
                """))
        conn.execute(text("""
                CREATE UNIQUE INDEX IF NOT EXISTS tenant_bots_org_slug_uniq
                ON tenant_bots (organization_id, slug)
                """))
        conn.execute(
            text("CREATE INDEX IF NOT EXISTS tenant_bots_status_idx ON tenant_bots (status)")
        )
        conn.execute(text(f"""
                CREATE TABLE IF NOT EXISTS tenant_bot_automations (
                    id {uuid_pk},
                    bot_id {uuid_col} NOT NULL,
                    organization_id {uuid_col} NOT NULL,
                    kind VARCHAR(32) NOT NULL,
                    name VARCHAR(120) NOT NULL,
                    mode VARCHAR(16) NOT NULL DEFAULT 'simulate',
                    enabled BOOLEAN NOT NULL DEFAULT FALSE,
                    cron VARCHAR(64),
                    config {json_type} NOT NULL DEFAULT '{{}}',
                    max_usd_per_run INTEGER NOT NULL DEFAULT 0,
                    max_usd_per_day INTEGER NOT NULL DEFAULT 0,
                    last_run_at {dt},
                    next_run_at {dt},
        ...
        conn.execute(
            text(
                "CREATE INDEX IF NOT EXISTS tenant_bot_automations_bot_idx "
                "ON tenant_bot_automations (bot_id)"
            )
        )
        conn.execute(text(f"""
                CREATE TABLE IF NOT EXISTS tenant_bot_runs (
                    id {uuid_pk},
                    automation_id {uuid_col} NOT NULL,
                    bot_id {uuid_col} NOT NULL,
                    idempotency_key VARCHAR(120) UNIQUE,
                    scheduled_for {dt},
                    status VARCHAR(16) NOT NULL,
                    reason TEXT,
                    spend_usd INTEGER NOT NULL DEFAULT 0,
        ...
        conn.execute(
            text(
                "CREATE INDEX IF NOT EXISTS tenant_bot_runs_bot_idx "
                "ON tenant_bot_runs (bot_id, started_at)"
            )
        )

    logger.info("Created tenant bot tables")
