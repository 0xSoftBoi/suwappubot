...
def test_pnl_uses_token_decimals_not_hardcoded_6(sqlite_db, monkeypatch):
    ...
    with get_session() as session:
        session.add(SwapTransaction(
            id=1, user_id=1,
            from_chain="ethereum", from_token="USDC", from_amount="2000000000",
            from_amount_usd=2000.0,
            to_chain="ethereum", to_token="WETH", to_amount=str(10**18),
            status="completed", tx_hash="0xabc",
        ))
