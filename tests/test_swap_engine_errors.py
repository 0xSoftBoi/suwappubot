...
def _make_quote(provider="lifi", to_amount_human=1.0, seconds_old=0):
    return SwapQuote(
        ...
        to_amount="500000000000000", to_amount_human=to_amount_human,
        to_amount_min="490000000000000",
        gas_cost_usd=0.5, fee_cost_usd=0.0, total_cost_usd=0.5,
        estimated_time=60, price_impact=0.1, exchange_rate=0.0005,
        raw_quote={"test": True},
...
def test_get_quote_raises_when_all_providers_fail(monkeypatch):
    ...
    for method in [
        "_get_lifi_quote", "_get_cctp_quote", "_get_across_quote",
        "_get_wormhole_quote", "_get_socket_quote", "_get_cow_quote",
        "_get_ccip_quote",
    ]:
        monkeypatch.setattr(engine, method, _fail)

    with pytest.raises((SwapError, Exception), match="(?i)no.*quote|provider"):
        asyncio.run(engine.get_quote(
            from_chain="ethereum", to_chain="base",
            from_token="USDC", to_token="WETH",
...
def test_get_quote_returns_best_when_some_providers_fail(monkeypatch):
    ...
    result = asyncio.run(engine.get_quote(
        from_chain="ethereum", to_chain="base",
        from_token="USDC", to_token="WETH",
        amount=1.0, amount_raw="1000000",
        from_address="0x" + "a" * 40,
...
def test_get_quote_raises_for_tron_cross_chain():
    ...
    with pytest.raises((SwapError, Exception), match="(?i)tron"):
        asyncio.run(engine.get_quote(
            from_chain="tron", to_chain="ethereum",
            from_token="USDT", to_token="USDC",
            amount=1.0, amount_raw="1000000",
            from_address="T" + "x" * 33,
        ))
...
def test_get_lifi_quote_raises_when_token_not_found(monkeypatch):
    ...
    with pytest.raises((SwapError, Exception)):
        asyncio.run(engine._get_lifi_quote(
            from_chain="ethereum", to_chain="base",
            from_token="FAKECOIN", to_token="WETH",
            amount=1.0, amount_raw="1000000",
            from_address="0x" + "a" * 40,
        ))
...
def test_execute_swap_raises_for_stale_quote(monkeypatch):
    ...
    with pytest.raises((SwapError, Exception), match="(?i)expir|stale|fresh"):
        asyncio.run(engine.execute_swap(
            quote=stale,
            wallet_id=1,
            user_id=1,
        ))
