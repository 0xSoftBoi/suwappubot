...
Used for `LibClone`'s "clone with immutable args" functions (`cloneDeterministic`,
`argsOnClone`, `initCodeHash`, `predictDeterministicAddress`) — the EIP-1167
minimal-proxy variant that appends constructor-style arguments to each clone's
own bytecode, read back via `extcodecopy` from the logic contract. Chosen over
hand-rolling this in `contracts/hypercore/RouterFactory.sol` because the
