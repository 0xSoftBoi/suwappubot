...
- [x] 3.1 DONE (db57893) — api-ts/src/aegis/ pure sync scanner: YAML loader (bundled + crypto copy, dedup, fail-open), regex matcher (unicode-normalized) + pure semantic heuristics. No ML. Was: Scanner core: YAML signature loader consuming the **same signature files** as Python (one source of truth in-repo) + the pure-string semantic heuristics. No ML.
