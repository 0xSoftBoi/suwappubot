...
| External resource created before durable local publication | Turnkey account discovery, cross-stack null→attested-ID enrichment, and managed-wallet metadata lease publication | Fixed with eventual-consistency retries, recoverable partial identity, one-way Python enrichment, and latest-metadata/token CAS | `turnkeyAgentWalletRecovery.test.ts`; enrichment tests in `test_internal_agent_wallet.py`; concurrent caller-metadata test in `agentWalletIdentity.test.ts` |
