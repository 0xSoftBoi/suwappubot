...
| Lease cleanup confuses retryable with ambiguous provider failure | Provider verify/create/address-validation, Python registration, and final metadata publication after a successful lease claim | Fixed with token-owned cleanup only before creation or after durable identity publication; ambiguous create/pre-publication outcomes retain the fail-closed marker to prevent remint | verification/finalization cleanup, replacement-token, ambiguous-create, publication, and invalid-address tests in `agentWalletIdentity.test.ts` |
| Cross-principal provider identity race | Two agents concurrently registering the same Turnkey org/wallet/account/address before either row exists | Fixed with sorted transaction-scoped advisory locks for every provider identity dimension plus conflict lookup on sub-org | `test_concurrent_agents_cannot_bind_the_same_provider_wallet`; provider-lock contention regression |
...
| Duplicated identity-value validator | Positive internal ID validation in metadata and Python response handling | Fixed by exporting and reusing the canonical metadata helper | managed-wallet metadata and route tests |
