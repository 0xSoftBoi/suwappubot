...
async def daily_checkin(request: Request):
    ...
    try:
        # Bonus fix: points_service.daily_checkin is a plain (sync) method,
        # not a coroutine — `await`ing it raised TypeError on every call. Call
        # it directly, and shape the tuple return into a named response.
        points_earned, streak, streak_continued, new_level = points_service.daily_checkin(
            payload["user_id"]
        )
        return {
        ...
        # Bonus fix: don't leak raw exception text to the client — consistent
        # with the classified error handling in the redeem route below.
        logger.error(f"Daily check-in failed for user {payload['user_id']}: {e}")
        raise HTTPException(status_code=400, detail="Check-in failed — please try again.")
...
async def redeem_reward(request: Request, reward_id: int):
    ...
    if reward_type in ("partner_transfer", "miles", "cashout", "stablecoin"):
        raise HTTPException(
            status_code=400,
            detail="That reward is coming soon — partner redemptions aren't live yet.",
        )
