...
# ==========================================
# Internal API — called by TypeScript API
# ==========================================
...
async def verify_internal_key(request: Request):
    ...
    if not INTERNAL_API_KEY or not key or key != INTERNAL_API_KEY:
...
class InternalSwapRequest(BaseModel):
    agent_id: int
    agent_uuid: str
    wallet_address: str
...
async def internal_provision_wallet(
    request: InternalProvisionRequest,
    _key: str = Depends(verify_internal_key),
):
    """Create a User + Wallet row for an agent. Called by TS API after Turnkey wallet creation."""
    import logging
    logger = logging.getLogger("suwappu.internal")

    try:
        # Generate a stable numeric ID from agent UUID
        ...
        logger.info(f"Provisioned wallet for agent {request.agent_uuid[:8]}: user_id={user_id}, wallet_id={wallet.id}, address={wallet.address}")
...
async def internal_execute_swap(
    request: InternalSwapRequest,
    _key: str = Depends(verify_internal_key),
):
    ...
    try:
        ...
        # Build SwapQuote from TS API's quote_data
        quote = SwapQuote(
        ...
        logger.info(f"Executing swap for agent {request.agent_uuid[:8]}: {quote.from_amount} {quote.from_token} → {quote.to_token} on {quote.from_chain}")

        # Execute via the full swap pipeline
