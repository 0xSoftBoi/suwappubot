...
ROUND 6 RESULT — "60 failing tests" WAS A PHANTOM (L11). With pytest-asyncio installed the suite is 1379 passed / 2 failed (reproduced twice). The original "60 failed" count came from a run WITHOUT pytest-asyncio: async tests in files that still collected fail with "async def not natively supported". So my own f7c7840d fix had ALREADY cleared ~58 of them; I kept quoting the stale number as outstanding work. LESSON: when a count comes from an earlier run, re-measure before treating it as a backlog — env changes invalidate old counts.
  Security/money-path files ALL PASS (swap_engine_wallet_ownership, p2p_escrow, oauth_link_binding, oauth_login_csrf, whatsapp_flows). Class A real product bugs: **0**.
  The 2 real failures = ONE root cause: api/main.py:865-876 computes JWT_SECRET at IMPORT time, so the first test file to import api.main freezes it process-wide. test_heartbeat_thresholds (alphabetically earlier) imports it with no SECRET_KEY set → random ephemeral secret → limit-orders' hardcoded tokens can never validate. test_webapp_referrals already had a defensive monkeypatch for this exact hazard; limit_orders never got it.
  PROD CHECK (me, before accepting "test-only"): api/main.py DOES log a warning on the ephemeral fallback, and railway.python-api.json has numReplicas:1 → no multi-replica secret divergence. Genuinely test-only, no prod bug.
  FIXED bff325a5 + VERIFIED BY CAUSATION TEST: same command, fix reverted = 2 failed/12 passed; fix applied = 14 passed.
ROUND 6 (superseded description):
...
ROUND 4 RESULT → issue #673. Mobile app broken 3 ways (all verified by me directly, not inferred):
  (1) mobile/lib/* (api.ts, auth.ts, passkey.ts, notifications.ts +6) exists ONLY on unmerged origin/claude/mobile-app-planning-Tmo3q. dev got hooks+screens but NOT the lib layer → all 10 hooks import '../lib/api' which doesn't exist → APP CANNOT BUILD. Partial merge dropped the data/auth layer.
  (2) eas.json production target = https://api.suwappu.xyz → curl 000, DOES NOT RESOLVE (.xyz vs real .bot). api.ts fallback = api.suwappu.bot which is api-ts, NOT the python service serving /v1/mobile/*.
  (3) packages/shared BaseApiClient calls bare /v1/alerts,/v1/orders,/v1/dca,/v1/copy/* but mobile router is prefix="/v1/mobile" → 404s. /v1/me family works only by coincidence (served by settings.py at bare /v1).
  Fix order: forward-port mobile/lib/* → fix eas.json host → reconcile prefix (careful: packages/shared is shared with webapp+api-ts).
ROUND 4 (superseded) cross-branch audit, origin/dev mobile client → fixed backend — especially RESPONSE-SHAPE mismatches (backend returns camelCase; does the app read keys the handler never returns?), incl. re-checking the just-fixed points/rewards/copy-trading handlers against app expectations.
