...
// PhosphorRace is a client component — next/dynamic is not needed here since
// the page is already a server component; the 'use client' in PhosphorRace
// will cause Next.js to split it into a client bundle automatically.
import { PhosphorRace } from '../next/PhosphorRace';
...
/*
 * HybridTerminalCard — wraps PhosphorRace in a dark rounded card that matches
 * the footprint of the LiveTerminal widget the classic hero used (~500-560px wide,
 * dark rounded panel with a slim title bar). The phosphor CSS vars are injected
 * here so the race renders correctly outside its native .page context.
 */
function HybridTerminalCard() {
  return (
    <div
      style={{
        // Inject phosphor design tokens so racePanel vars resolve correctly
        '--bg':     '#161210',
        '--panel':  '#1F1915',
        '--signal': '#E86A2C',
        '--glow':   '#FF8A3D',
        ...
        '--dim':    '#6E6157',
        '--keys':   '#4E8C5A',
        '--rule':   'rgba(110, 97, 87, 0.35)',
        // Font stack: PhosphorRace expects JetBrains Mono; the terminal card
        // falls back gracefully to Courier New on first render.
        fontFamily: "var(--font-jb, 'JetBrains Mono'), 'Courier New', monospace",
        // Card shell: rounded, dark bg, subtle border — mirrors LiveTerminal
        width: '100%',
        maxWidth: '560px',
        background: '#161210',
        borderRadius: '12px',
        overflow: 'hidden',
        boxShadow: '0 8px 40px rgba(0,0,0,0.55), 0 0 0 1px rgba(110,97,87,0.25)',
        flexShrink: 0,
      } as React.CSSProperties}
    >
      {/* Slim chrome title bar — signals this is a live terminal widget */}
      <div
        style={{
          display: 'flex',
          alignItems: 'center',
          gap: '0.6rem',
          padding: '0.6rem 1rem',
          borderBottom: '1px solid rgba(110,97,87,0.3)',
          background: '#1A1512',
        }}
      >
        {/* Traffic-light dots */}
        <span style={{ display: 'flex', gap: '5px', alignItems: 'center' }}>
          <span style={{ width: 10, height: 10, borderRadius: '50%', background: '#E86A2C', boxShadow: '0 0 5px rgba(232,106,44,0.5)' }} />
          <span style={{ width: 10, height: 10, borderRadius: '50%', background: '#3D3530' }} />
          <span style={{ width: 10, height: 10, borderRadius: '50%', background: '#3D3530' }} />
        </span>
        <span
          style={{
        ...
        >
          ...
          <span style={{ color: '#E86A2C' }}>·</span>{' '}
          best-route execution
        </span>
      </div>

      {/* The race itself — clip the racePanel's 4rem margin-bottom so the card
          sits flush. The inner div collapses that trailing margin. */}
      <div style={{ padding: '0.25rem 0 0', overflow: 'hidden' }}>
        <PhosphorRace />
      </div>
    </div>
  );
...
async function Hero() {
  ...
  return (
    <section className="summer-hero">
      ...
      {/* PhosphorRace replaces LiveTerminal as the hero's visual centerpiece */}
      <HybridTerminalCard />
    </section>
  );
}
