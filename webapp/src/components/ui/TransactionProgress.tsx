...
export type TransactionStatus = 'idle' | 'submitting' | 'pending' | 'confirming' | 'confirmed' | 'completed' | 'failed'
...
export function TransactionProgressInline({ status }: { status: TransactionStatus }) {
  ...
  const statusConfig = {
    ...
    completed: { text: 'Completed', color: 'text-suwappu-success', icon: '✓' },
  ...
  return (
    ...
    >
      <motion.span
        animate={status !== 'confirmed' && status !== 'completed' && status !== 'failed' ? { rotate: 360 } : {}}
