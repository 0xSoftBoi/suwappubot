...
STAKING = "0xFA1142C788b6BC09CD16490dFEdAcEAFC505bA17"
BONDS = "0x9aCCf607AF27327B4940827a5c389F109847562D"
...
def send(fn, label):
    ...
    tx = fn.build_transaction(
        {"from": addr, "nonce": nonce, "gasPrice": int(w3.eth.gas_price * 1.2), "chainId": CHAIN_ID}
    )
...
if not suwp.functions.hasRole(MINTER, Web3.to_checksum_address(STAKING)).call():
    send(
        suwp.functions.grantRole(MINTER, Web3.to_checksum_address(STAKING)),
        "grantRole MINTER -> Staking",
    )
else:
    print("  · Staking already has MINTER_ROLE")
if not suwp.functions.hasRole(MINTER, Web3.to_checksum_address(BONDS)).call():
    send(
        suwp.functions.grantRole(MINTER, Web3.to_checksum_address(BONDS)),
        "grantRole MINTER -> Bonds",
    )
else:
