...
def expected_fingerprint(ref: str) -> str:
    try:
        ...
        raise SystemExit(2)
    paths = sorted(
        p for p in listing if p.endswith(".py") and p.split("/", 1)[0] in HASHED_DIRS and "__pycache__" not in p.split("/")
    )
...
def main() -> int:
    ...
    if stale:
        print(f"\n✗ running code does not match {args.ref}: {', '.join(stale)}.", file=sys.stderr)
        return 1
