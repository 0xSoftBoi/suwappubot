/**
 * Network layer for Gecko, the Suwappu mobile client.
 *
 * Talks to Suwappu's existing end-user API surface, but adds four things that
 * matter far more on a phone than in a browser tab:
 *
 *  1. In-flight deduplication. Three components mounting at once and each
 *     asking for the same snapshot should produce ONE request, not three. On LTE
 ...
 *     Writes are never retried by default.
 *  4. ETag-based conditional requests for large, slow-changing reads.
 *
 * Every call is timed through perf.ts so slow endpoints surface as data.
 */
import { API_BASE_URL, TIMEOUTS } from './config'
import { getAuthRevision, getAuthToken, setAuthToken } from './auth'
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401 && getAuthToken()) {
          // An expired/revoked session must not leave cached financial data on
          // screen. setAuthToken also clears user-scoped Query/HTTP caches.
...
export function request<T>(endpoint: string, opts: RequestOptions = {}): Promise<T> {
  ...
  // The revision is deliberately not the bearer token itself. It keeps
  // in-flight dedupe scoped to one signed-in user without persisting a secret.
  const key = `${getAuthRevision()} ${method} ${endpoint}`
